...
void loop()
{
  ...
  dist_ema = dist_raw*_EMA_ALPHA + (1.0-_EMA_ALPHA)*dist_ema;// Put EMA filter code here
      
  //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
  duty = (dist_ema - _DIST_MIN) / (_DIST_MAX - _DIST_MIN) * (_DUTY_MAX - _DUTY_MIN) + _DUTY_MIN; // Put map() equivalent code here
